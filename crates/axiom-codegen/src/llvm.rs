...
fn emit_extern_function_decl(ctx: &mut CodegenContext, ef: &HirExternFunction) {
    ...
    // Map calling convention to LLVM CC keyword.
    let cc = match ef.convention.as_str() {
        "C" | "" => "",
    ...
    // Emit @axiom_link comments for any @link annotations on this extern function.
    for ann in &ef.annotations {
        if let HirAnnotationKind::Link { ref library, ref kind } = ann.kind {
            let _ = writeln!(ctx.output, "; @axiom_link: {} {}", library, kind);
        }
    }
}
...
fn emit_expr(ctx: &mut CodegenContext, expr: &HirExpr, expected_type: Option<&str>) -> LlvmValue {
    match &expr.kind {
        HirExprKind::IntLiteral { value } => {
            ...
            // When a pointer type is expected and the literal is 0, emit `null`
            // instead of the integer `0` (LLVM requires `ptr null`, not `ptr 0`).
            let reg = if ty == "ptr" && *value == 0 {
                "null".to_string()
            } else {
                format!("{value}")
            };
...
fn emit_call(ctx: &mut CodegenContext, func: &HirExpr, args: &[HirExpr]) -> LlvmValue {
    if let HirExprKind::Ident { name } = &func.kind {
        ...
        match name.as_str() {
            ...
            // Input/Audio builtins removed — use extern "C" fn + @link instead.
        ...
        // Determine calling convention: fastcc for internal, or extern convention.
        let cc = if func_info.uses_fastcc {
            "fastcc "
        } else {
            match func_info.convention.as_str() {
                "fastcall" => "x86_fastcallcc ",
                "stdcall" => "x86_stdcallcc ",
                "win64" => "win64cc ",
                _ => "",
...
// Renderer/GPU builtins removed — use extern "C" fn + @link instead.
// See lib/renderer.axm, lib/gpu.axm, lib/input.axm, lib/audio.axm.
